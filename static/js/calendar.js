...
(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        define('calendar', ['jquery.min.js'], factory);
    ...
    var defaults = {
            ...
            // 切换字符
            prev: '<img src="/Images/assets/img/Nxin/leftca.png" />',
            next: '<img src="/Images/assets/img/Nxin/rightca.png" />',
        ...
        MARK_DAY_HTML_1 = '<i class="dot_1"></i>',
        MARK_DAY_HTML_2 = '<i class="dot_2"></i>',
        DATE_DIS_TPL = '{year}年<span class="m"> {month}月</span>',
        //DATE_DIS_TPL = '<span class="m"> {month}月</span>',
    ...
    function abc(){
        ...
        $.ajax({
            ...
            success: function (data) {
                console.log(data);
                //var length_1 = data[0].length;
                //var length_2 = data[1].length;
                var zxm = new Array();

                for (var z in data[0]) {
                    for (var c in data[0]) {
                        if (data[0][z] == data[1][c]) {
                            zxm[num_array] = data[0][z];
                            num_array++;
                        }
                    }
                }
                ...
                for (var i in data[0]) {
                    if (data[1].length == 0) {
                        if (i == (data[0].length - 1)) {
                            mark_data += '{"date":"' + data[0][i] + '","flag":"0"}';
                        } else {
                            mark_data += '{"date":"' + data[0][i] + '","flag":"0"},';
                        }
                    } else {
                ...
                for (var s in data[1]) {
                    if (zxm.length == 0) {
                        if (s == (data[1].length - 1)) {
                            mark_data += '{"date":"' + data[1][s] + '","flag":"1"}';
                        } else {
                            mark_data += '{"date":"' + data[1][s] + '","flag":"1"},';
                        }
                    } else {
                ...
                mark_data = JSON.parse(mark_data);
                console.log(mark_data);
    ...
    Calendar.prototype = {
        ...
        getDayData: function (day) {
            var ret, data = this.data, flag;

            if (data) {

                for (var i = 0, len = data.length; i < len; i++) {
                    var item = data[i];//循环查询标记项目
                    //console.log(item);
        ...
        getDayItem: function (y, m, d, f) {
            ...
            if (markData) {
                ...
                if (markData == 0) {
                    $item.html(d + MARK_DAY_HTML_1);
                } else if (markData == 1) {
                    $item.html(d + MARK_DAY_HTML_2);
                } else if (markData == 2) {
                    $item.html(d + MARK_DAY_HTML_1 + MARK_DAY_HTML_2);
                }
                //$item.html(d + MARK_DAY_HTML_1);
            }
        ...
        getDisDateValue: function () {
            ...
            var y = Number(arr[0]);
            var m = Number(arr[1].match(/\d{1,2}/)[0]);
